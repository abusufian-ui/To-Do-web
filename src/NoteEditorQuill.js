...
const NoteEditor = ({ courses = [], onBack, initialNote = null, onSave, onDelete }) => {
  ...
  // --- NEW: INTERCEPT IMAGE PASTING (CTRL+V) ---
  useEffect(() => {
    ...
    const handlePaste = async (e) => {
      ...
      if (e.clipboardData && e.clipboardData.files && e.clipboardData.files.length > 0) {
        ...
        // If it is an image, hijack the paste event!
        if (file.type.startsWith('image/')) {
          e.preventDefault(); // Stop Quill from pasting the giant Base64 string
          
          setSaveStatus('Uploading pasted image...');
          ...
          try {
            ...
            if (response.ok && data.url) {
              ...
              // Insert the clean Cloudinary URL into the editor
              quill.insertEmbed(range.index, 'image', data.url);
              // Move the cursor to right after the newly inserted image
              quill.setSelection(range.index + 1);
              
              setIsDirty(true);
              setSaveStatus('Unsaved changes');
            } else {
              console.error("Server rejected pasted image:", data);
    ...
    // Attach the event listener directly to the Quill editor's root DOM element
  ...
  imageResize: {
      ...
      modules: ['Resize', 'DisplaySize'] // 'Resize' gives the drag handles, 'DisplaySize' shows the pixel dimensions while dragging
    },
  history: {
    delay: 200,
    maxStack: 500,
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white dark:bg-[#121212] overflow-hidden relative">
      ...
      /* QUILL CODE BLOCK LANGUAGE DROPDOWN THEME FIX */
        ...
        /* Force a blue border on images when they are clicked */
.quill-wrapper .ql-editor img:active,
.quill-wrapper .ql-editor img:focus {
