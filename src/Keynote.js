...
import {
  ...
  X, Download, Maximize2, Type, EyeOff, CheckSquare, Square, Lock, Globe
} from 'lucide-react';
import UCPLogo from './UCPLogo';
import EmptyState from './EmptyState';

const Keynote = ({ keynotes = [], courses = [], onToggleRead, onDelete, onBatchDelete, user }) => {
  ...
  const [sharedKeynotes, setSharedKeynotes] = useState([]);
  ...
  const togglePrivacy = async (e, note) => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/api/keynotes/${note._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'x-auth-token': token },
        body: JSON.stringify({ isPrivate: !note.isPrivate })
      });
      setSharedKeynotes(prev => prev.map(k => k._id === note._id ? { ...k, isPrivate: !note.isPrivate } : k));
      if (selectedNote && selectedNote._id === note._id) {
        setSelectedNote({ ...selectedNote, isPrivate: !note.isPrivate });
      }
    } catch (error) { console.error("Error toggling privacy", error); }
  };
        ...
        <div className="flex items-center gap-3">
          {displayKeynotes.length > 0 && (
            <button
              ...
              className={`px-4 py-2 rounded-xl border font-semibold text-sm transition-all ${isSelectMode
          ...
          <div className="text-sm bg-white dark:bg-[#222230] px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center gap-2">
            <span className="text-brand-blue font-bold text-lg leading-none">{displayKeynotes.filter(k => !k.isRead).length}</span>
            ...
            {displayKeynotes.map((note) => {
              ...
              return (
                <div
                  ...
                  className={`relative group flex flex-col h-auto min-h-[200px] rounded-2xl p-5 border cursor-pointer transition-all duration-300 ${isSelectMode && isSelected
                ...
                >
                  ...
                  {viewMode === 'shared' && note.userId && (
                    <div className="text-[10px] text-blue-500 font-bold mb-1">By {note.userId.name}</div>
                  )}
          ...
          <div className="relative bg-white dark:bg-[#1c1c24] w-full max-w-2xl rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col max-h-[90vh] overflow-hidden transform animate-slideUp">
            ...
            <div className="p-5 sm:p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-start bg-gray-50 dark:bg-[#222230]">
              ...
              <div className="flex items-center gap-2">
                {((selectedNote.userId ? (typeof selectedNote.userId === 'object' ? selectedNote.userId._id === user?.id : selectedNote.userId === user?.id) : true) || user?.isAdmin) && (
                  <button onClick={() => togglePrivacy(null, selectedNote)} className="p-2 text-gray-500 dark:text-gray-400 hover:text-indigo-500 rounded-full transition-all" title={selectedNote.isPrivate ? "Make Shared" : "Make Private"}>
                    {selectedNote.isPrivate ? <Lock size={18} /> : <Globe size={18} />}
                  </button>
                )}
